...
export function JobCard({ job, isOverlay = false }: JobCardProps) {
  ...
  const baseClasses = `bg-white p-4 rounded-lg border border-slate-200 shadow-sm transition-all duration-200 ${
    isOverlay ? 'shadow-2xl rotate-3 scale-105 cursor-grabbing border-blue-200' : 'cursor-grab hover:border-blue-300'
  }`;
  ...
  if (isDragging && !isOverlay) {
    return (
      <div 
        ref={setNodeRef}
        style={style}
        className="bg-slate-200/30 border-2 border-dashed border-slate-300 rounded-lg p-4 h-[110px] transition-all"
      />
    );
  }
