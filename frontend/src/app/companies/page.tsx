...
export default function CompaniesPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    const newCompany = await createCompany({ name, website });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Empresas Cadastradas</h1>
        <p className="text-slate-500 mt-1">Gerencie as empresas para as quais você se candidatou</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 space-y-1 w-full">
            ...
            <input
              ...
              required
              className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
            />
          </div>
          <div className="flex-1 space-y-1 w-full">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-semibold transition-colors shadow-sm disabled:opacity-50 h-[42px]"
          >
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="text-center p-12 bg-slate-100/50 rounded-xl border-2 border-dashed border-slate-200">
            <p className="text-slate-500 font-medium">Nenhuma empresa cadastrada ainda.</p>
          </div>
        )}
      </div>
    </div>
